...
export function useMenuItemFilters(data?: MenusConfig): MenuItemFiltersResult {
  if (data && (!Array.isArray(data.menus) || !data.menus.every(menu => 'menu' in menu))) {
    throw new Error('Invalid menu configuration provided');
  }

  const [filter, setFilter] = useState<string>(FILTER_TYPES.ALL);
  const [filteredData, setFilteredData] = useState<MenusConfig | undefined>(data);
